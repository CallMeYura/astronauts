...
**Лемма**
Пусть задано семейство функции A ⊄ M; 0, 1 ∈ [A]. Тогда (не)x ∈ [A]
...
Следовательно, (не)x. Лемма доказана
...
Вопрос 25: Графы: вершины, ребра, инцидентность. Изображение и изоморфизм графов
-----------------------------------------------------------------------------------------------------
...
Явное задание графа как алгебраической системы. – <{a,b,c,d}; {(a,b), (b,a),(b,c),(c,b),(a,c),(c,a),(c,d),(d,c)}>. Чтобы задать такое представление, достаточно для каждого ребра указать двухэлементное множество вершин.
Геометрический – как на картинке.
Матрица смежности – это лог матрица отношения на множестве вершин простого графа, которое задается его ребрами. Т.е. отображает есть ли между вершинами ребра связ. их. Размер V*V.
Матрица инцидентности – лог матрица отражающая инцидентность вершин и ребер. Т.е. указывает связь между вершиной и ребром.
-----------------------------------------------------------------------------------------------------
Вопрос 26: Подразделение графа, подграф, гомеоморфизм графов. Планарность графа
-----------------------------------------------------------------------------------------------------
Подграфом графа называется граф, являющийся подмоделью исходного графа. Иначе говоря, подграф содержит некоторые вершины исходного графа и некоторые рёбра. Подразделение графа G — это граф, полученный делением рёбер в G. Например, ребро ⅇ с вершинами u, v: может быть разделено на ребро e1и e2.
...
Вопрос 27: Пути, циклы, петли на графе. Связность графа и степени его вершин
-----------------------------------------------------------------------------------------------------
Путь в графе — последовательность вершин, в которой каждая вершина соединена со следующей ребром. Длиной пути называется число ребер этого пути. Путь называется простым, если он не проходит через одну вершину более одного раза.
...
Петля в графе — ребро, инцидентное одной и той же вершине. Граф без петель — это простой граф. Обычно у петли нет ориентации.
...
* Компонентом связанности графов называется множество точек для каждой пары которых имеется связывающий путь.
  Вопрос 28: Оценка числа графов с заданным количеством ребер
-----------------------------------------------------------------------------------------------------
...
Вопрос 31: Двухполюсные сети. Суперпозиция сетей. П – сети
-----------------------------------------------------------------------------------------------------
...
Вопрос 33: Формула Эйлера для плоских графов и ее следствия
-----------------------------------------------------------------------------------------------------
...
ⅇ=v,v – петля. Тогда из графа G удалим это ребро, получим граф, у которого p вершин, q-1 ребер, m-1 граней. Но поскольку по индукционному предположению верна формула Эйлера, то
p+m–1=q–1+2. Прибавив 1 слева и справа, получим нашу формулу: p+m = q+2.
ⅇ=v,w где v≠w. Удаляем ребро e, получим граф H. Возможны два случая:
Граф H – связный, значит, мы можем прийти из вершины v в вершину w по другому маршруту. У графа H p вершин, (q–1) ребер и (m–1) граней, т.е. число граней уменьшается, так как вершины v и w лежат в разных гранях, а после удаления ребра е грани сливаются. Мы получили случай, аналогичный первому.
Граф H – несвязный, т.е. после удаления ребра е получаем 2 связные компоненты графа H1H2:  H1=p1,q1,m1, H2=p2,q2,m2 Для H1 и H2 справедлива формула Эйлера по индукционному предположению p1m1=q1+2, P2m2=q2+2, p1+p2=p, q1+q2=q-1, m1+m2=m+1, так как вместо одной бесконечной грани стало 2. Сложим наши равенства:
P1+P2+m1+m2=q1+q2+4.
p+m+1 = q–1+4                        p+m = q+2.
Теорема доказана.
...
Вопрос 34: Теорема о пяти красках для плоских графов
-----------------------------------------------------------------------------------------------------
...
Вопрос 35: Основные понятия теории кодирования: алфавит, слово, сообщение, схема кодирования, декодирование
-----------------------------------------------------------------------------------------------------
...
Вопрос 39: Префиксные коды и деревья. Способы уменьшения избыточности кода
-----------------------------------------------------------------------------------------------------
